...
**What happens to unused relief.** SRS contributions can be invested inside the SRS account across a fairly wide range of instruments. That range now includes Ireland-domiciled index funds on SGX, which matters for anyone whose SRS money holds US-domiciled funds: [what changed from 13 October 2026](/sgx-irish-etfs-us-estate-tax/). CPF top-ups earn the CPF interest rates on the relevant account, which for many people is difficult to beat on a risk-adjusted basis, but comes with far less flexibility in how the money is used.
